---
title: SlimmeLezer Mini
date-published: 2026-04-24
type: sensor
standard: global
board: esp32
difficulty: 1
made-for-esphome: true
project-url: https://github.com/zuidwijk/SlimmeLezer/tree/main/Mini
---

![SlimmeLezerMini](SlimmeLezer-Mini.png "SlimmeLezerMini")

## Description

SlimmeLezer Mini, an ESP32C3 based device to read out the P1 port of a Smart Meter and send data via wifi to Home Assistant.

## Basic Configuration

The full yaml code can be found at
[https://github.com/zuidwijk/SlimmeLezer/blob/main/Mini/config.yaml](https://github.com/zuidwijk/SlimmeLezer/blob/main/Mini/config.yaml)
